...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            IEnumerable<Emp> result = Emps.Where(e => e is { Job: "Frontend programmer", Salary: > 1000 })
        ...
        {
            var result = Emps.Where(e => e.Job == "Frontend programmer").MaxBy(e => e.HireDate) ?? throw new InvalidOperationException();
            return result;
        }
        ...
        public static IEnumerable<object> Task10()
        {
            ...
            IEnumerable<object> result = Emps.Select(e => new { e.Ename, e.Job, e.HireDate }).Union(defaultValue);;
            return result;
        }
        ...
        {
            return Emps.TaskGetEmpsWithSubordinates();
        }
        ...
        {
            return arr.GroupBy(x => x).Where(g => g.Count() % 2 != 0).Select(g => g.Key).FirstOrDefault();
        }
        ...
        public static IEnumerable<Dept> Task14()
        {
            IEnumerable<Dept> result = Depts.GroupJoin(Emps,
                ...
                .Select(x => x.Dept)
                .OrderBy(d => d.Dname);
            //result = Dept: IT, Location: Los Angeles
            return result;
        }
        ...
        public static IEnumerable<object> Task15()
        {
            IEnumerable<object> result = Emps.Where(e => e.Job.Contains("A"))
                ...
                .Where(g => g.Count() > 2)
                .Select(g => new { Praca = g.Key, LiczbaPracownikow = g.Count() })
                .OrderByDescending(g => g.LiczbaPracownikow);
            //result =
            return result;
        }
        ...
        {
            IEnumerable<object> result = Emps.Join(Depts, e => e.Deptno, d => d.Deptno, (e, d) => new { Emp = e, Dept = d });
            //result =
            return result;
        }
    }
    ...
    public static class CustomExtensionMethods
    {
        ...
        public static IEnumerable<Emp> TaskGetEmpsWithSubordinates(this IEnumerable<Emp> em)
        {
            var emps = em as Emp[] ?? em.ToArray();
            var mgrIds = new HashSet<int>(emps.Where(e => e.Mgr != null).Select(e => e.Mgr.Empno));  // Pobranie identyfikatorów menedżerów.
            return emps.Where(e => mgrIds.Contains(e.Empno))
                .OrderBy(e => e.Ename)  
